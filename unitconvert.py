...
def toMev(before,unit_before):
    ...
    if(unit_before=='Hz'):
        return hbar*before*2*np.pi/(1e6*e)
    if(unit_before=='kHz'):
        return hbar*before*2*np.pi/(1e3*e)

def toRadius(before,unit_before):
    if(unit_before=='beta'):
        return mass_sun*Gc2*before/100000

def toMass(before,unit_before):
    if(unit_before=='radius_cm'):
        return before/(Gc2*mass_sun)

def toBeta(before,unit_before):
    if(unit_before=='mass_sun_cm'):
        return mass_sun*Gc2*before
    if(unit_before=='mass_sun_m'):
        return mass_sun*Gc2*before/100
    if(unit_before=='mass_sun_km'):
        return mass_sun*Gc2*before/100000
